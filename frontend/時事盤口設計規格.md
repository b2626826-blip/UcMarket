...
> 目前狀態：時事市場已改用 `GET /api/current-affairs/markets`，首頁承接列表，`/markets/current-affairs` 會導向 `/home`，詳情保留 `/markets/current-affairs/:id`。以下內容主要是視覺與互動規格；Mock 階段與「後端完成後再替換」的步驟已屬歷史，不可當成現況。
