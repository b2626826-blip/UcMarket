...
| 認證機制 | HttpOnly Cookie + Refresh Token     | 管理登入狀態、session 與續登入流程            |
| 資料存取 | Spring Data JPA / Hibernate         | 操作使用者、session、市場與角色相關資料       |
| 資料庫   | PostgreSQL                          | 儲存 users、user_sessions、markets 與相關資料 |
| 前端     | HTML / CSS / JavaScript / Bootstrap | 製作後台登入頁、使用者管理頁與 Market 管理頁  |
...
3. 使用者登入成功後，系統建立 session，並透過 HttpOnly Cookie 回傳登入狀態。
4. Refresh token 對應資料會記錄於 user_sessions，方便續登入與 session 撤銷管理。
5. 使用者登出時，系統清除對應 Cookie，並使 session 失效。
...
Tim 主要負責 UcMarket 最核心的交易與動態定價機制，並將其實作拆分為以下兩大核心功能：

#### 1. 交易試算功能

本功能主要提供使用者在實際下單前的即時反饋機制。預測市場的 Yes / No 價格會隨著群體交易行為產生動態變更，系統將透過 Java 核心語法實作自動做市商(AMM)演算法來動態計算即時單價。此功能透過 Spring MVC 設計成標準的 RESTful API，讓前端頁面在使用者輸入欲購買金額時，能透過 JavaScript / jQuery 的非同步請求(AJAX / Fetch)，即時回傳預估買入份數與成交滑點，優化互動體驗。

#### 2. 下單交易功能

本功能負責執行最終的交易撮合與狀態變更，屬於數據一致性要求極高的核心邏輯。當交易發起時，需透過 Spring 框架的 `@Transactional` 確保「扣除使用者錢包虛擬點數」、「建立或更新持倉紀錄（採加權平均法計算持倉均價）」與「寫入交易流水帳」具備原子性(Atomicity)。針對熱門議題可能引發的多人同時下單情境，亦負責於 Hibernate / JPA 層導入資料庫鎖定機制，防止資產超賣、帳目髒讀等併發問題，確保底層資料正確。

Shung 主要負責 UcMarket 的 User/Auth 與 Market 功能。User/Auth 功能會處理使用者註冊、登入、登出、登入狀態查詢與角色權限控管，作為平台的基礎入口；Market 功能則負責市場建立、查詢、編輯與審核流程，並搭配後台頁面完成管理操作，讓平台能順利銜接後續交易、結算與排行榜等功能。

Roy 主要負責 UcMarket 的持倉功能與前端版面風格設計。持倉功能會在使用者完成交易後建立或更新 positions 資料，並計算持倉數量、平均成本、目前盈虧與持倉狀態，供資產頁、結算功能與排行榜功能使用。前端設計部分則先以 Figma 製作 wireframe，統一整體 CIS、版面配置與 HTML 頁面風格，協助各組員頁面在整合時保持一致。
