...
| ROY | 持倉功能 | 後端功能 | 持倉中同一方向的持倉，則更新原本持倉；若沒有則新增一筆持倉。,持倉大部分會去抓取別人的欄位及運算,除了盈虧,需要套用額外公式及邏輯運算,最後再去更新持倉最新狀態。 |設計持倉 餘額 總額 盈虧 持倉狀態 結算 |盈虧邏輯運算 抓取其他欄位資料(錢包/市場/交易) 加倉邏輯 

| ROY | 頁面 | 前端功能 | 利用FIGMA製作WIREFRAME後設計整體CIS並製作出HTML頁面。 |設計CIS把所有每個人的頁面做出來  
...
2.| Roy | 前端頁版面與風格設計| 負責前端的版面設計,利用figma wireframe排版再把版面製作成html頁面| 6/12完成 |  進行中 | 
