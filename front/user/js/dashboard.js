(function () {
  ...
  var slide, slideIndex, timeText, marketGrid, searchInput, tabButtons;
  ...
  function renderHeroSlide() {
    if (!slide) return;
    var item = heroSlides[slideIndex];
    slide.innerHTML = '<span class="badge">' + item.badge + '</span><h1>' + item.title + '</h1><p>' + item.text + '</p><div class="hero-buttons"><button class="primary-btn">' + item.primary + '</button><button class="secondary-btn">' + item.secondary + '</button></div>';
  }
  ...
  function updateTaipeiTime() {
    if (!timeText) return;
    var now = new Date();
    timeText.textContent = now.toLocaleString('zh-TW', { timeZone: 'Asia/Taipei', year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit' });
  }
  ...
  function renderMarkets(data) {
    ...
    data.forEach(function(market) {
      ...
      card.innerHTML = '<div class="market-header"><h4>' + market.title + '</h4><span>' + market.date + '</span></div><div class="market-price"><div class="yes"><small>YES</small><h3>$' + market.yesPrice.toFixed(2) + '</h3><button class="trade-btn buy-btn" data-id="' + market.id + '" data-side="YES">Buy Yes</button></div><div class="no"><small>NO</small><h3>$' + market.noPrice.toFixed(2) + '</h3><button class="trade-btn sell-btn" data-id="' + market.id + '" data-side="NO">Buy No</button></div></div><div class="market-footer"><span>Volume ' + market.volume + '</span><span>' + market.traders + ' traders</span></div>';
  ...
  function filterMarkets() {
    ...
    var filtered = marketData.filter(function(market) {
      return (currentCategory === '全部' || market.category === currentCategory) && (market.title.toLowerCase().includes(keyword) || market.category.toLowerCase().includes(keyword));
    });
  ...
  function initMarketSection() {
    ...
    tabButtons = document.querySelectorAll('.market-tabs button');
    if (!marketGrid) return;
    ...
    document.addEventListener('click', function(e) {
      ...
      var market = marketData.find(function(item) { return item.id === id; });
      if (market) {
    ...
    setInterval(function() {
      marketData.forEach(function(market) {
        var move = Math.random() * 0.04 - 0.02;
        market.yesPrice += move;
        if (market.yesPrice > 0.95) market.yesPrice = 0.95;
        if (market.yesPrice < 0.05) market.yesPrice = 0.05;
  ...
  window.initDashboard = function () {
    initHeroSlider();
    initChart();
    timeText = document.getElementById('taipeiTime');
    if (timeText) {
      updateTaipeiTime();
