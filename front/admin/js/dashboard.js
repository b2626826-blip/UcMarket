(function () {
  ...
  var mockData = {
    ...
    quickActions: [
      { label: "建立新事件", hint: "建立新的預測事件", page: "views/create-market.html" },
      { label: "查看所有事件", hint: "管理所有事件", page: "views/markets.html" },
      { label: "用戶清單", hint: "管理平台用戶", page: "views/users.html" },
      { label: "系統設定", hint: "調整系統參數", page: "views/settings.html" }
    ],
    systemInfo: [
  ...
  function renderQuickActions(actions) {
    ...
    container.innerHTML = actions.map(function (a) {
      return '<a class="quick-action-btn" href="#" data-page="' + a.page + '">' +
        '<div class="fw-semibold mb-1">' + a.label + '</div>' +
        '<div class="small text-secondary">' + a.hint + '</div>' +
      '</a>';
    }).join('');
  }
