...
- `01`、`05`、`06` 只允許一台監控哨啟用，避免 Discord 重複告警。
...
匯入後必須仍為 inactive，並確認清單只有 `01-health-alert`、`04-notify-webhook`、`05-failed-alert`、`06-heartbeat`；不得出現 `02`／`03`。
...
`ucmarket-n8n-service-token` 的 value 必須與 backend 的 `N8N_SERVICE_TOKEN` 相同；`ucmarket-notify-webhook-token` 必須與 `N8N_NOTIFY_WEBHOOK_TOKEN` 相同。只確認兩端已由同一次安全輸入更新，不讀取或回顯 value。
...
4. 每條 workflow 啟用後，確認下一次正式排程 execution 正常。
5. 檢查 Discord／Mailpit 實際結果，再記錄 execution ID、時間與結果。
...
不得保存 secret、credential value、recipient、API response body 或含敏感資料的 execution snapshot。
