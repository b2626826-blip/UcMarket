...
1. **照 id 覆蓋、不會長重複**——JSON 內帶 workflow id，重跑幾次都是同五條。（UI 的 Import from File 每次生一條新的、雙告警——別用它來「更新」，只在 CLI 不可用時救急）
...
- **狀態**：workflow JSON、隔離 fixture、兩組 credential 綁定與 n8n `2.29.11` fresh session runtime 驗收均已完成（無 findings）。版控匯出為 `active: false`；這不代表任一部署的即時 runtime 狀態。每個環境都須確認指定的唯一 `07` instance 後再決定是否 Activate。
...
- **秘密與 execution 界線**：兩組 value 必須不同，只能存在 backend 安全啟動介面與各 n8n instance credential store；不得進 workflow JSON、`install/credentials.json`、文件、execution snapshot 或 log。workflow 固定 `saveDataErrorExecution=all`、`saveDataSuccessExecution=none`、`saveManualExecutions=false`、`saveExecutionProgress=false`：成功 execution 不保存；失敗 execution 保留供排錯，再由 pruning 依保留期清理。可見 execution 以 `deletedAt IS NULL` 判斷。
