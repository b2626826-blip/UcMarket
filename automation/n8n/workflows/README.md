...
## 02-daily-digest 每日摘要（需要後端關卡 3 API）

- **做什麼**：8–23 點每整點**醒來檢查**——今天還沒發過就 `GET /api/internal/notifications/digest`（昨日）→ 發一則 📊 摘要到 Discord（新市場／待審／成交量）。**一天恰一發**（`lastSentDate` 判重）：正常日 08:00 準點；8 點時後端或 n8n 剛好掛，之後任一整點自動補發。
- **憑證**：`discord-ucmarket-每日摘要`＋`ucmarket-service-token`（Header Auth：Name `X-Service-Token`、Value 與後端 `service.token` 一致）
- **前置**：後端已部署關卡 3（`/api/internal/**`＋SecurityConfig 放行提案已合）；service token 憑證已入庫（見下方「service token 憑證怎麼來」）
- **驗法**（先 Activate）：確認後端活著 → 等下一個整點（≥8 點）應恰見一則摘要、數字與資料庫對帳 → 同日之後的整點不再發 → `docker compose restart n8n` 跨過整點 → 下個整點補發
- **坑**：同 `01`——判重記在 staticData，手動 Execute 不保存；且**執行中途失敗（API 掛、DC 掛）當次不記「已發」**，下一小時自動重試，這是故意的。

## 03-closing-reminder 截止提醒（需要後端關卡 3 API）

- **做什麼**：**每 30 分** `GET /api/internal/notifications/closing-markets?withinHours=24` → 過濾已提醒過的（`sentKeys`，鍵＝`DEADLINE_REMINDER:{marketId}:24H`，市場截止即自動清鍵）→ 有新的**彙總成一則** ⏰ 發 Discord。與核心線的截止提醒不撞：他寄 email 給持倉使用者（個人信），這條是 DC 頻道廣播。
- **憑證**：`discord-ucmarket-市場關閉`＋`ucmarket-service-token`（同 `02` 那筆）
- **驗法**（先 Activate）：造一個 23 小時後截止的 ACTIVE 市場 → 下個整點恰一則提醒 → 再下一小時不重貼 → 造 31 小時後的 → 不觸發；SYS- 系統建立者的市場不會出現
- **坑**：同 `02` 的 staticData 兩條。

## 05-failed-alert FAILED 告警（需要後端關卡 3 API）

- **做什麼**：每 10 分鐘 `GET /api/internal/notifications/failed`——重試耗盡（FAILED 終態）的通知數量**有變化且 >0** 就發一則 ⚠️（含前 5 筆明細）。同數量不重複轟炸；清到 0 自動重置。這條解「Email 管線壞了、告警也走 Email」的死結——告警走 Discord。
- **為何不吃 admin API**：`/api/admin/**` 要 admin JWT，n8n 得養機器帳號；改在 internal 面自建唯讀 `/failed`，用同一把 service token，零擴權。
- **憑證**：`discord-ucmarket-寄信狀態`＋`ucmarket-service-token`
- **驗法**（先 Activate、`notification.worker.enabled=true`）：停掉 mailpit 或 `04` → 觸發一筆通知讓它重試耗盡變 FAILED → 10 分鐘內恰一則 ⚠️ → 再等 10 分鐘不重貼 → 管理後台重送成功後歸零
- **坑**：同前——staticData 手動 Execute 不保存。

### service token 憑證怎麼來（02/03/05 共用）

`install/credentials.json` 已內含 `ucmarket-service-token`（id 固定 `UcSvcTokenHdr001`）——新機器跑 setup（或 `install\import-credentials.ps1`）匯入後**綁定自動生效**，不用手動建。前提：後端 `application.properties` 的 `service.token` 與它同值（開發預設值已對齊；正式部署兩邊一起用環境變數換）。
