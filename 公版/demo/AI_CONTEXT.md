...
目前 `公版/demo` 資料夾是一份獨立靜態前端 prototype，主要用途是給團隊建立共同視覺參照。它不是正式 React 前端，也沒有串接後端 API。
...
│   └── AI_CONTEXT.md         # 本文件
└── user
    ├── index.html            # 使用者端首頁靜態頁
    ├── css/style.css
    ├── js/main.js
    └── wallet
        ├── view/index.html   # 使用者錢包頁
        ├── css/style.css
        └── js/wallet.js
```
...
- `rankings.html`：排行榜頁，含盈虧、勝率、資產等排行方向。
...
2. 修改主要 demo prototype 時，優先在 `公版/demo/` 內處理；修改使用者端拆頁時，才進入 `公版/user/`。
3. 保持黑底 Apple-like 風格：乾淨、留白、少色彩、低噪音。
4. 不要把頁面改成行銷 landing page；這是產品操作介面的參考公版。
5. 卡片圓角維持小圓角，避免過度可愛或遊戲化。
6. 若增加功能，優先補足預測市場產品需要的頁面狀態，例如 loading、empty、error、pending review、resolved。
7. 若串接後端，讓 quote 與 trade 以 API 結果為準，不要只依賴前端假資料。
8. 若轉成 React，先保留目前視覺與資訊架構，再逐步拆 component。
