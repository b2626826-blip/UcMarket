...
export default function TradePanel({ marketId, market, side, onSideChange }) {
  ...
  const shares = resolveDisplayedShares({ quotedOdds, amountNumber, liveOdds });
...
function resolveDisplayedShares({ quotedOdds, amountNumber, liveOdds }) {
  const quotedShares = Number(quotedOdds?.shares);
  if (Number.isFinite(quotedShares) && quotedShares > 0) {
    return quotedShares;
  }

  return liveOdds > 0 ? amountNumber / liveOdds : 0;
}
