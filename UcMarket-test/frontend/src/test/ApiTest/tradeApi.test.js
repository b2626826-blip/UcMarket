...
describe('tradeApi.js', () => {
  ...
  it('createTrade POST /api/trades with idempotency header', async () => {
    await createTrade({ marketId: 'm1', amount: 10 }, 'trade-123');
    const [url, options] = fetchMock.mock.calls[0];
    expect(url).toBe(apiUrl('/api/trades'));
    expect(options.method).toBe('POST');
    expect(options.body).toBe(JSON.stringify({ marketId: 'm1', amount: 10 }));
    expect(options.headers['Idempotency-Key']).toBe('trade-123');
  });
