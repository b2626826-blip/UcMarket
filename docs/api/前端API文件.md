...
> 來源：`frontend/src/api/` 全部模組、`frontend/src/pages/public/market-detail-weather/weatherApi.js` 與現行測試（2026-07-21 整理）。

## 共用 HTTP Client — `src/api/client.js`

所有 API 模組共用的 fetch 封裝：

- `BASE_URL`：空字串，使用同源 `/api`；Vite dev server 以 proxy 轉送到 `http://localhost:8080`。
...
- 錯誤處理（`handleResponse`）：
  - `401` → 清除 token 並導向 `/auth/login`。
  - `403` → 保留 token，丟出帶 `status=403` 的 Error，讓頁面顯示權限錯誤。
...
| `forgotPassword(email)` | POST `/api/auth/forgot-password` | 申請密碼重設 |
| `resetPassword(token, newPassword)` | POST `/api/auth/reset-password` | 使用重設 token 更新密碼 |
| `deleteAccount(password)` | DELETE `/api/auth/me` | 刪除目前帳號；密碼可選 |
...
| `getMarketPriceHistory(id, from, to)` | GET `/api/markets/{id}/price-history` | 依可選 `from`／`to` 取得價格歷史 |
...
| `getMyMarkets({page, size})` | GET `/api/markets/me` | 目前登入者送審／上架／結束中的市場 |
| `searchTradingViewSymbols(keyword)` | TradingView symbol search | 金融市場建立頁的 symbol 搜尋；失敗時回退本機 catalog |
...
| 函式 | 呼叫端點 | 功用 |
|---|---|---|
| `getPositions()` | GET `/api/positions/me` | 取得目前登入者全部持倉 |
| `getOpenPositions()` | GET `/api/positions/me/open` | 取得目前登入者未結算持倉 |
| `getMarketPositions(marketId, {openOnly})` | GET `/api/positions/market/{marketId}` 或 `/open` | 取得指定市場全部或未結算持倉 |
...
| `adjustWallet(id, body)` | POST `/api/admin/users/{id}/wallet/adjust` | 管理員調整使用者錢包 |
| `getUserWallet(id)` | GET `/api/admin/users/{id}/wallet` | 取得使用者餘額與近期流水 |
...
- [settings/index.jsx](../../frontend/src/pages/admin/settings/index.jsx)：直接 `fetch('/api/health')`（相對路徑、未經 client.js，dev 環境依賴 Vite proxy 或同源部署）。
...
2. **`tradeApi.getTrades()`** 用 GET `/api/trades`；後端 `/api/trades` 只有 POST。此 wrapper 目前只被測試引用。
3. （次要）`marketApi.getMarkets()` 傳 `status=ACTIVE` query 參數，後端 `GET /api/markets` 固定只回 ACTIVE，該參數會被忽略。
