...
### 階段一．五：n8n 監控告警（已實作，待 `05` 實跑驗收）

Docker／Mailpit 環境、`01-health-alert`、`06-heartbeat` 與 n8n 查詢專用 service token 已完成。`05-failed-alert` workflow JSON 已實作，尚待各 n8n instance 建立並綁定 credential 後做正式排程驗收：

1. **FAILED 告警**：每 5 分鐘以 service token 輪詢 WP4 `GET /api/admin/notifications?status=FAILED`；每頁 `size=100` 並依 `hasNext` 讀完所有頁面。
2. **防重複**：staticData 保存 `jobId → attemptCount`；只有完整查詢且 Discord 成功後才更新快照。同一次 FAILED 不重複告警，job 離開後再次 FAILED 會重新告警。
3. **職責邊界**：連線失敗與 HTTP 5xx 不由 `05` 發 Discord，避免重複 `01-health-alert`；401／403 或 API 契約錯誤只讓該次 execution 失敗。
...
上述差異已同步回《自動化系統規劃》與《自動化系統分工計畫》。WP0–WP5、n8n 實際寄送橋接、規則式預審後端第一切片與 n8n 監控線 service token 均已完成；`05-failed-alert` JSON 已實作，待 credential 綁定與正式排程實跑驗收。
