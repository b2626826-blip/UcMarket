...
UcMarket 採 Java／Spring Boot 與 n8n 分層：Java 負責交易一致性、排程、outbox、冪等與重試；n8n 負責實際寄送、監控告警與外部服務整合。核心可靠性不得依賴 n8n。
...
- 寄信失敗不得影響市場核准、交易或結算。
- n8n 只透過 `EmailSender` adapter 或受保護的 REST API 接入，不直接讀寫 PostgreSQL。
...
- Java 已完成 WP0–WP5：通知 outbox、Worker、重試、冪等、管理員查詢／重送及六類市場事件通知；後續密碼重設也沿用同一 outbox。
- `automation/n8n/workflows` 已有 `01-health-alert`、`04-notify-webhook`、`05-failed-alert`、`06-heartbeat`、`07-resolution-evidence-collector` 五條 workflow。
- 後端已完成正式的 `N8nWebhookEmailSender` 與 webhook token、URL、timeout 設定；通知 Worker 預設停用，啟用後會走 Java outbox → n8n → SMTP 的實際寄送路徑。
- 規則式預審後端第一切片已完成：V10 `market_review_checks`、五項確定性規則及送審交易整合均已有測試。
...
| Email | `EmailSender` 介面搭配 `N8nWebhookEmailSender`；SMTP 由 n8n 端負責 |
...
已新增獨立的 `market_review_checks` 保存自動檢查結果，不直接把系統檢查塞進既有 `market_reviews`。既有資料表繼續保存真正的人工核准、駁回與意見。
...
│   ├── N8nWebhookEmailSender.java
│   └── NotificationJobRepository.java
└── service/
    └── MarketPreReviewService.java
```

核心自動化程式放在後端；`automation/n8n` 保留 Docker、安裝腳本、workflow JSON 與 runbook，作為外部整合層的版控真相源。
...
### 第三階段：規則式預審（後端第一切片已完成）

實作內容：

- 建立 `market_review_checks`（已完成）。
- 建立第一批硬性規則（已完成）；風險標記規則待判斷標準明確後另做。
- 後台顯示檢查項目、結果與原因（待後續工作包）。
- 保留管理員最終核准權限。
...
核准／駁回／要求修改、管理員摘要、截止提醒及結算通知亦已於 WP5 完成；`N8nWebhookEmailSender` 及 Java outbox → n8n → Mailpit 的失敗重試、恢復補送與恰好一次寄送亦已實跑驗收。規則式預審後端第一切片亦已完成，下一步是後台查詢／顯示檢查結果；預審累積足夠人工結果後，再根據誤判率決定是否加入 AI 或有限度自動核准。
...
- 送審、核准、駁回、要求修改與結算已集中在 `MarketService`，狀態變更與 enqueue 共用 transaction。
- PostgreSQL、Spring Data JPA 與 Flyway 已啟用；目前 migration 到 `V13`。
- WP0–WP5 已完成：`notification_jobs`／attempts、Worker、`RecordingEmailSender`、模板、管理員查詢／重送及六類市場事件通知均已有測試；密碼重設通知另由後續測試覆蓋。
- n8n 已完成環境、健康／FAILED 告警、心跳、`POST /webhook/notify` 與時事市場結算蒐證；Java main code 已由 `N8nWebhookEmailSender` 接通寄送路徑。
- 現有 `CreateMarketRequest` 只保證標題、截止時間及 URL 格式等基本輸入驗證，不能取代規則式預審。

### 13.2 WP0–WP5 已採用的實作決策
...
4. 第一個切片先使用可記錄寄件內容的假寄信器；正式寄送已由 `N8nWebhookEmailSender` 接通。
5. 通知 payload 保存產生通知當下所需的不可變快照；模板不得在寄送時重新推導市場舊狀態。
6. WP0–WP5 未新增 Kafka、RabbitMQ、Redis、Python Worker 或額外服務；n8n 保持在核心交易之外，作為外部寄送與整合層。

### 13.3 資料庫設計

`V6__add_notification_jobs.sql` 建立通知 outbox 並加入 `submission_version`；`V8__add_market_submission_version.sql` 再以 `IF NOT EXISTS` 確保不同既有資料庫升級路徑收斂。每次由 `DRAFT` 進入 `PENDING` 時遞增，以區分退回修改後的再次送審。
...
- `V6__add_notification_jobs.sql` 建立 outbox 並加入 `submission_version`；後續 V8 以 `IF NOT EXISTS` 確保不同既有資料庫升級路徑收斂。
...
已新增 `N8nWebhookEmailSender` 作為 `EmailSender` 的 main-code 實作，呼叫既有 `POST /webhook/notify`。webhook URL、token 與 timeout 由設定注入；2xx 視為成功，4xx、5xx、timeout 及連線錯誤會拋出例外，交由既有 Worker 記錄 attempt 並進入 `RETRY`／`FAILED`。

本工作包不得修改 outbox schema、Worker 狀態機、WP5 event／payload／模板，也不得新增 `/api/internal/notifications/digest`、`/closing-markets` 或 n8n `02`／`03` 排程；每日摘要與截止提醒已由 Java WP5 排程建立通知工作，重做會造成兩套排程與重複通知。

驗收：

1. sender 定向測試涵蓋 request body、token header、2xx、4xx／5xx、timeout 與連線失敗。
2. Worker 關閉時應可正常啟動；Worker 開啟但缺少必要 sender 設定時應 fail fast。
3. n8n 停機時通知工作進入 `RETRY` 並留下 attempt；恢復後轉為 `SENT` 且只寄一次。
4. n8n `04-notify-webhook` 實跑後，Mailpit 恰好收到一封正確主旨與內容的 Email。

2026-07-19 驗收結果：`N8nWebhookEmailSenderTest` 10 項通過；測試通知留下 `1:RETRY, 2:SENT` 兩筆 attempt，最終狀態為 `SENT`；Mailpit 恰好收到一封正確信件；backend 全測試共 332 項，0 failures、0 errors、15 skipped。

### 13.6 規則式預審（後端第一切片已完成）

V10 已建立 `market_review_checks`，並以 `market_id`、`submission_version`、`rule_code`、`rule_version` 唯一鍵保存每次送審的可追溯結果。第一批只做五項可完全確定的規則：必填欄位、截止時間必須晚於檢查時間、HTTP(S) 來源、資料庫欄位長度，以及目前僅支援 `BINARY`／YES-NO 選項。重複市場、來源可信度、政治／法律風險尚無明確判斷標準，本切片不實作、不誤標 `BLOCKED`。

送審時先執行預審，再變更市場狀態與建立通知；任一規則為 `BLOCKED` 時保存檢查結果但市場維持 `DRAFT`，其他預審例外則整筆交易回滾。管理員仍保留最終核准權限；後台查詢／顯示檢查結果另列下一工作包。
...
2026-07-19 驗收結果：預審與送審定向測試共 50 項通過；既有資料庫成功由 V9 升至 V10，一次性空資料庫成功套用 V1～V10；backend 全測試共 343 項，0 failures、0 errors、15 skipped。

### 13.7 時事市場結算蒐證（已完成）

V12 建立 `market_resolution_evidence`；候選讀取與證據寫入使用兩組互斥的最小權限 service token。n8n `07-resolution-evidence-collector` 每小時先完成最多 4 頁／200 筆候選快照，再逐市場登記既有 `sourceUrl`；不抓取來源內容、不自動結算。成功 execution 不保存，失敗 execution 保留供排錯。

### 13.8 每個工作包的共同完成條件

1. 先跑該工作包的定向測試。
2. 再於 `backend/` 執行 `./mvnw test`。
3. 若工作包新增 migration，同時驗證「全新資料庫」與「上一版本資料庫升級」。
