...
### 13.0 執行狀態（2026-07-16）

| 工作包 | 狀態 | 依據／下一步 |
|---|---|---|
| WP0 | 未完成 | `MarketController.submitMarket()` 仍直接改狀態及存檔；下一步先把交易邊界移入 `MarketService`。 |
| WP1 | PASS | 空 PostgreSQL V1→V6、歷史 V1→V5 再升目前 V6、冪等 enqueue 均已由 fresh session 驗收。 |
| WP2 | PASS | 成功／失敗 attempt、1／5／30 分鐘重試、FAILED、雙 Worker、lease 回收及錯誤截斷均已由 fresh session 驗收。 |
| WP3 | 未開始 | 依賴 WP0、WP1、WP2；WP0 完成後再串接市場送審通知。 |
| WP4 | 未開始 | 已具備 repository 契約，可與 WP0／WP3 主線平行，但不列為下一個單一步驟。 |

目前下一個單一步驟是 **WP0：交易邊界與送審入口**。本輪仍不處理 WP3、WP4、WP5 或 n8n。
...
- PostgreSQL、Spring Data JPA 與 Flyway 已啟用；目前 migration 到 `V9`，其中 `V6` 建立通知工作與寄送嘗試資料表。
- 已有通知 Entity、Repository、冪等 enqueue、假寄信器、Worker、重試與 attempt 紀錄；尚未從市場送審流程建立通知工作，也尚無正式寄送 Provider。
