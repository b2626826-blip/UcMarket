...
- Java 已完成 WP0–WP5：通知 outbox、Worker、重試、冪等、管理員查詢／重送及六類市場事件通知。
- `automation/n8n/workflows` 已有 `01-health-alert`、`04-notify-webhook`、`06-heartbeat` 三條 workflow。
...
- PostgreSQL、Spring Data JPA 與 Flyway 已啟用；目前 migration 到 `V10`。
- WP0–WP5 已完成：`notification_jobs`／attempts、Worker、`RecordingEmailSender`、模板、管理員查詢／重送及六類事件通知均已有測試。
- n8n 已完成環境、健康告警、心跳及 `POST /webhook/notify` 收信端；Java main code 已由 `N8nWebhookEmailSender` 接通正式寄送路徑。
...
`V6__add_notification_jobs.sql` 建立通知 outbox；`V8__add_market_submission_version.sql` 再為 `markets` 加入 `submission_version INTEGER NOT NULL DEFAULT 0`。每次由 `DRAFT` 進入 `PENDING` 時遞增，以區分退回修改後的再次送審。
...
- `V6__add_notification_jobs.sql`；`submission_version` 由後續 `V8__add_market_submission_version.sql` 建立。
