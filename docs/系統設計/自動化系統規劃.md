...
- 後端尚缺正式的 `N8nWebhookEmailSender`；通知 Worker 預設停用，因此目前仍未接通 Java 到 n8n 的實際寄送路徑。
...
核准／駁回／要求修改、管理員摘要、截止提醒及結算通知亦已於 WP5 完成。下一步先接通 `N8nWebhookEmailSender`，讓既有 outbox 真正送到 n8n；之後才進入規則式預審。預審累積足夠人工結果後，再根據誤判率決定是否加入 AI 或有限度自動核准。
...
- PostgreSQL、Spring Data JPA 與 Flyway 已啟用；目前 migration 到 `V9`。
- WP0–WP5 已完成：`notification_jobs`／attempts、Worker、`RecordingEmailSender`、模板、管理員查詢／重送及六類事件通知均已有測試。
- n8n 已完成環境、健康告警、心跳及 `POST /webhook/notify` 收信端；Java main code 尚無 `EmailSender` 正式實作。
...
4. 第一個切片使用可記錄寄件內容的假寄信器；正式寄送由下一工作包的 `N8nWebhookEmailSender` 接通。
...
### 13.5 下一工作包：n8n 實際寄送橋接

新增 `N8nWebhookEmailSender` 作為 `EmailSender` 的 main-code 實作，呼叫既有 `POST /webhook/notify`。webhook URL、token 與 timeout 必須由設定注入；2xx 視為成功，4xx、5xx、timeout 及連線錯誤必須拋出例外，交由既有 Worker 記錄 attempt 並進入 `RETRY`／`FAILED`。
...
通知垂直切片穩定後才新增下一版 migration（目前應由 `V10` 起）建立 `market_review_checks`。第一批只做可完全確定的規則：必填欄位、截止時間、HTTP(S) 來源、欄位長度與 market type／選項相容性。重複市場、來源可信度、政治／法律風險先標 `NEEDS_REVIEW`，不做 `BLOCKED`。
