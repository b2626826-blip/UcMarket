...
> 更新基準：前端路由以 `frontend/src/router/routes.jsx`、API 以 controller 與 `docs/api/`、schema 以 Flyway V1～V13、n8n 以 workflow JSON 為準。
...
flowchart TB
    ...
    scheduler -->|Webhook| n8n
    n8n -->|Email / Discord| user
```
...
- 排程、outbox、冪等與重試留在 Spring Boot；n8n 只負責外部寄送、監控與蒐證。
...
| `/auth/forgot-password`、`/auth/reset-password` | `frontend/src/pages/public/*password*` | 密碼重設 |
...
| 我的市場 | GET | `/api/markets/me` | 個人市場管理 |
...
| 價格歷史 | GET | `/api/markets/{id}/price-history` | 類別市場詳情圖表 |
| 管理市場 | GET | `/api/admin/markets` | 管理員頁，可依 status 篩選 |
...
| 通知工作 | GET | `/api/admin/notifications?status=...` | 管理通知失敗與重送 |
| 結算證據 | GET | `/api/admin/markets/{id}/resolution-evidence` | 管理員結算判讀 |
...
| 交易列表 | `GET /api/trades` 或專用 `/me` 路由 | 前端 wrapper 已宣告，後端仍只有 POST |
...
sequenceDiagram
    ...
    B->>D: 預審並轉為 PENDING、建立通知 outbox
    A->>F: 進入管理員頁
    F->>B: GET /api/admin/markets?status=PENDING
    B->>D: 查詢待審核市場
    B-->>F: 回傳待審核列表
    A->>F: 通過或拒絕
...
| 我的資產 | `wallets`, `positions`, `trades`, `market_price_history` |
| 排行榜 | `users`, `wallets`, `wallet_transactions`, `trades`, `positions`, `markets`, `market_price_history` |
| 通知 | `notification_jobs`, `notification_job_attempts` |
| 規則預審／結算證據 | `market_review_checks`, `market_resolution_evidence` |

正式 schema 以 Flyway V1～V13 為準；`docs/資料庫設計/ucmarket-ddl.sql` 尚未包含 V6～V13。

## 11. Java／Spring Boot 自動化位置

自動化位於後端 Service、`automation`、`notification` package 與 `automation/n8n/`。排程可以查詢並觸發工作，但交易、錢包、持倉或結算資料仍只能由既有 Service 修改。

目前已有市場截止、價格 snapshot、天氣市場、通知 outbox／Worker、每日摘要、截止提醒、管理員重送與 n8n 五條 workflow；完整邊界見自動化與 n8n 規劃文件。
...
1. 決定並補齊 `GET /api/trades` 契約，或移除未使用的前端 wrapper。
2. 補 OpenAPI 與可重現的整體 E2E 驗證。
3. 完成規則式預審結果的管理員查詢／顯示。
4. 驗證各主題詳情、OAuth、密碼重設、錢包、交易與管理後台的 production flow。
...
| Spring 自動化 | 已接通主要流程 | 排程、outbox、Worker、重試、摘要、截止提醒與事件通知已完成 |
| n8n 整合 | 已有五條 workflow | `01/04/05/06/07`；匯出檔 active 狀態不等同任一部署 runtime |
