...
- `公版/` 是獨立靜態 prototype，可作為正式前端開發時的畫面與流程參考。
...
完整架構圖原始檔：`docs/系統設計/網站架構圖.mmd`

若需要放進簡報或 README，可使用同資料夾下的 `網站架構圖.svg` 或 `網站架構圖.png`。

![UcMarket 網站架構圖](./網站架構圖.svg)

網站頁面規劃心智圖：`docs/系統設計/網站規劃圖.mmd`

若需要像 XMind 的網站規劃圖，可使用同資料夾下的 `網站規劃圖.svg` 或 `網站規劃圖.png`。
...
`公版/` 目前已提供靜態頁面，可對應成正式前端路由：

| 正式路由 | 參考頁面 | 用途 |
|---|---|---|
| `/` | `公版/index.html` | 首頁、市場列表、搜尋、分類、熱門市場 |
| `/auth` | `公版/auth.html` | 登入與註冊 |
| `/markets/:id` | `公版/market-detail.html` | 市場詳情、價格走勢、交易面板 |
| `/markets/new` | `公版/create-market.html` | 建立市場與規則檢查 |
| `/portfolio` | `公版/portfolio.html` | 錢包、持倉、交易紀錄 |
| `/rankings` | `公版/rankings.html` | 使用者排行榜、市場排行榜 |
| `/admin/markets` | `公版/admin.html` | 管理員審核與結算 |

正式前端開發時，不建議直接把 `公版/` 當成正式前端，而是將其中的版型、互動與資訊架構搬進 `frontend/src`。
...
| 排行榜 | `user_portfolio_snapshots`, `trades`, `positions` |
| 通知 | `notifications` |
...
| 排行榜 API | 尚未完成 | DDL 可支援，需補後端查詢 |
| n8n | 規劃中 | 應作為周邊通知與報表 |

## 14. 結論

UcMarket 的網站架構應以 `公版/` 作為產品與畫面參考，以 `frontend/` 作為正式前端實作位置，並透過 Spring Boot REST API 串接 PostgreSQL。核心交易、錢包、持倉與結算必須留在後端與資料庫交易中處理；前端負責提供清楚、快速、可操作的交易與管理介面。
