...
> 文件定位：課程專題企劃與分工原稿。本文的預定日期與功能描述不等同目前完成度；現況以 controller、router、Flyway migration、workflow JSON 與測試為準。自動化現採 Java／Spring Boot 核心可靠性加 n8n 外部整合的分層架構。
...
資料庫負責保存會員、市場、交易、持倉、錢包、價格歷史、人工／規則審核、通知 outbox、結算證據與管理操作紀錄。正式 schema 由 Flyway V1～V13 管理。

#### 5. Java／Spring Boot 自動化流程

Spring Boot 已負責市場截止、價格 snapshot、天氣市場、通知 outbox／Worker、失敗重試、每日摘要與截止提醒；n8n 負責通知 webhook、監控告警、心跳與時事市場結算蒐證。核心交易與結算仍只由 Service transaction 修改。
...
截至 2026-07-21，Auth／OAuth／密碼重設、Market、BUY Trade、Wallet、Position、Resolution、Ranking、主要管理 API、React 頁面、價格歷史讀取、通知 outbox／n8n 寄送、GCP Docker Compose 與 workflow 07 蒐證皆已有實作。仍明確存在的契約缺口是前端 `GET /api/trades` wrapper；OpenAPI、站內通知、多選項／SELL 與完整 E2E 仍屬後續。下表保留原始預定日期與分工，不再作為目前完成度來源。
