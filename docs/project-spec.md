...
| result | enum | yes / no / null |
| yes_pool | numeric | Yes 流動池 |
| no_pool | numeric | No 流動池 |
| created_at | timestamp | 建立時間 |
...
| user_id | uuid | 使用者 ID |
| market_id | uuid | 市場 ID |
| yes_shares | numeric | Yes 份額 |
| no_shares | numeric | No 份額 |
| updated_at | timestamp | 更新時間 |
...
| id | uuid | 紀錄 ID |
| market_id | uuid | 市場 ID |
| yes_price | numeric | Yes 價格 |
| no_price | numeric | No 價格 |
| volume | numeric | 成交量 |
